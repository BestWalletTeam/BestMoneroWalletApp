        ...
        , m_lockFile(nullptr)
{
    ...
    QString identifier = "Best Wallet";
    if (!userName.isEmpty()) {
        identifier += "-" + userName;
    }
...
void Application::loadFonts() {
    for (const QString &font : {QStringLiteral(":/assets/fonts/Inter.ttf"),
                                QStringLiteral(":/assets/fonts/JetBrainsMono.ttf")}) {
        if (QFontDatabase::addApplicationFont(font) < 0) {
            // The stylesheets name a generic family beside each.
