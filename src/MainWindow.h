...
#ifndef BESTWALLET_MAINWINDOW_H
#define BESTWALLET_MAINWINDOW_H
...
private:
    ...
    // Change leftba active widget as navigation changes
    void updateNavigationState(QWidget* active, QPushButton *activeButton);
    QString formatStatusBalance(const QString &amount, const QString &suffix);
    void applyThemedIcons();
    // Status bar artwork, normalised to a common optical size.
    QIcon statusGlyph(const QString &name, bool tint) const;
    QIcon statusDot(const QString &name) const;
    void updateLeftbarLogo();

    // Last figure shown in the status bar. formatStatusBalance bakes theme
    // colours into the markup, so a skin change has to re-render it.
    QString m_statusBalanceAmount = "0";
    QString m_statusBalanceSuffix = "XMR";
