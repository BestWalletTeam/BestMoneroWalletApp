...
void WindowManager::startupWarning() {
    // Stagenet / Testnet
    auto worthlessWarning = QString("Best Wallet is currently running in %1 mode. This is meant "
...
void WindowManager::onWalletOpened(Wallet *wallet) {
    ...
    if (status != Wallet::Status_Ok) {
        ...
        if (status == Wallet::Status_BadPassword) {
            ...
            Utils::Message message{nullptr, Utils::ERROR, "Unable to open wallet", errMsg, {"You may need to give Best Wallet permission to access the folder", "In the System Settings app, go to 'Privacy & Security' -> 'Files & Folders'"}};
#else
...
void WindowManager::tryCreateWallet(Seed seed, const QString &path, const QString &password, const QString &seedLanguage,
    ...
    wallet->setAppAttribute(WalletAttributes::seed, seed.mnemonic.join(" "));
    wallet->setAppAttribute(WalletAttributes::seedOffset, seedOffset);
...
void WindowManager::handleDeviceError(const QString &error, Utils::Message &msg) {
    ...
    if (error.contains("No device found")) {
        ...
        msg.helpItems = {"Make sure the Monero app is open on the device.", "If the problem persists, try restarting Best Wallet."};
...
void WindowManager::buildTrayMenu() {
    ...
    menu->addSeparator();
    menu->addAction("Exit Best Wallet", this, &WindowManager::close);
}
...
void WindowManager::onProxySettingsChanged() {
    ...
    if (conf()->get(Config::proxy).toInt() != Config::Proxy::None) {
        ...
        if (conf()->get(Config::proxy).toInt() == Config::Proxy::Tor && (!torManager()->isLocalTor() || torManager()->isAlreadyRunning())) {
            host = torManager()->managedTorHost;
            port = torManager()->managedTorPort;
        }
...
void WindowManager::initSkins() {
    // Each skin is a complete stylesheet, so switching swaps the whole look.
    m_skins.insert(constants::skinNativeDark, this->loadStylesheet(":/assets/style/brand.qss"));
    m_skins.insert(constants::skinNativeWhite, this->loadStylesheet(":/assets/style/native_white.qss"));

    QString skin = conf()->get(Config::skin).toString();
    if (!m_skins.contains(skin)) {
        // Carries over configs written before the themes were named, plus any
        // of the removed QDarkStyle/Breeze skins.
        skin = constants::skinNativeDark;
        conf()->set(Config::skin, skin);
    }

    this->applyThemeInk(skin);
    qApp->setStyleSheet(m_skins[skin]);
}

void WindowManager::applyThemeInk(const QString &skinName) {
    // Glyph colour for monochrome icons, matching each theme's primary text.
    icons()->setInk(skinName == constants::skinNativeWhite ? QColor("#3A4250")
                                                           : QColor("#FFFFFF"));
}
...
void WindowManager::onChangeTheme(const QString &skinName) {
    ...
    // The wizard is not in m_windows, and its menu page paints a brand mark a
    // stylesheet swap cannot reach.
    if (m_wizard) {
        m_wizard->skinChanged();
    }
}
