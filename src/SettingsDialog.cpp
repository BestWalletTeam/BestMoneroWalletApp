        ...
        , m_nodes(nodes)
{
    ...
    ui->selector->setIconSize(QSize(26, 26));

    new QListWidgetItem(QIcon(), "Appearance", ui->selector, Pages::APPEARANCE);
    new QListWidgetItem(QIcon(), "Network", ui->selector, Pages::NETWORK);
    new QListWidgetItem(QIcon(), "Storage", ui->selector, Pages::STORAGE);
    new QListWidgetItem(QIcon(), "Display", ui->selector, Pages::DISPLAY);
//  new QListWidgetItem(QIcon(), "Memory", ui->selector, Pages::MEMORY);
    new QListWidgetItem(QIcon(), "Transactions", ui->selector, Pages::TRANSACTIONS);
    new QListWidgetItem(QIcon(), "Plugins", ui->selector, Pages::PLUGINS);
    new QListWidgetItem(QIcon(), "Misc", ui->selector, Pages::MISC);

    this->refreshSelectorIcons();
...
//    }
    ...
    // Widgets nested inside the tab pages are built before the pages are
    // adopted, so they miss the "#Settings ..." rules until this runs.
    Utils::repolishRecursive(this);

    this->adjustSize();
}

void Settings::refreshSelectorIcons() {
    // Row order matches the Pages enum used when the items were created.
    static const QStringList glyphs = {
        "settings_appearance.svg", "settings_network.svg", "settings_storage.svg",
        "settings_display.svg", "settings_transactions.svg", "settings_plugins.svg",
        "settings_misc.svg",
    };

    for (int i = 0; i < ui->selector->count() && i < glyphs.size(); ++i) {
        ui->selector->item(i)->setIcon(icons()->tinted(glyphs.at(i)));
    }
}
...
void Settings::setupAppearanceTab() {
    ...
    connect(ui->comboBox_theme, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int pos){
        ...
        // The dialog stays open across the switch, so recolour its own glyphs.
        this->refreshSelectorIcons();
    });
...
void Settings::setupNetworkTab() {
    ...
    if (m_nodes) {
        ...
        connect(ui->nodeWidget, &NodeWidget::connectToNode, m_nodes, QOverload<const NodeEntry&>::of(&Nodes::connectToNode));
    } else {
...
void Settings::setupThemeComboBox() {
    // Only the built-in themes remain, so there is nothing to prune here.
