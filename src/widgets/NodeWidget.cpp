...
void NodeWidget::onShowWSContextMenu(const QPoint &pos) {
    ...
    NodeEntry node = this->selectedNode();
...
void NodeWidget::showContextMenu(const QPoint &pos, const NodeEntry &node) {
    QMenu menu(this);
...
NodeEntry NodeWidget::selectedNode() {
    QModelIndex index = m_activeView->currentIndex();
    if (!index.isValid()) return NodeEntry();
...
void NodeWidget::onContextCustomNodeRemove() {
    ...
    NodeEntry node = m_customModel->node(index.row());

    auto nodes = m_nodes->customNodes();
    QMutableListIterator<NodeEntry> i(nodes);
    while (i.hasNext())
        if (i.next() == node)
            i.remove();
...
void NodeWidget::onCustomAddClicked(){
    ...
    for (auto &newNodeText: newNodesList) {
        ...
        auto node = NodeEntry(newNodeText);
        node.custom = true;
