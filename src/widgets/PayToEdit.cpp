...
void PayToEdit::updateSize() {
    ...
    // Room the frame, document margin and stylesheet padding need on top of the
    // text. Measured from live geometry, since a fixed allowance collapses the
    // viewport as soon as the active stylesheet's padding exceeds it.
    int chrome = this->height() - this->viewport()->height();
    if (chrome <= 0) {
        chrome = int(2 * this->document()->documentMargin()) + 2 * this->frameWidth();
    }

    int h = int(docHeight * lineHeight) + chrome + 2;
