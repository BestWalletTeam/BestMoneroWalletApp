...
#ifndef BESTWALLET_NODES_H
#define BESTWALLET_NODES_H
...
struct NodeEntry {
    explicit NodeEntry(QString address = "", int height = 0, int target_height = 0, bool online = false)
            : height(height)
...
public:
    ...
    QList<NodeEntry> customNodes();
    QList<NodeEntry> websocketNodes();
...
public slots:
    void connectToNode();
    void connectToNode(const NodeEntry &node);
    void onWSNodesReceived(QList<NodeEntry>& nodes);
    void onNodeSourceChanged(NodeSource nodeSource);
    void setCustomNodes(const QList<NodeEntry>& nodes);
...
private:
    ...
    QList<NodeEntry> m_customNodes;
    QList<NodeEntry> m_websocketNodes;

    NodeEntry m_connection;  // current active connection, if any
    ...
    NodeEntry pickEligibleNode();

    bool useOnionNodes();
    bool useI2PNodes();
    bool useSocks5Proxy(const NodeEntry &node);

    void resetLocalState();
    void exhausted();
    int modeHeight(const QList<NodeEntry> &nodes);
};

#endif //BESTWALLET_NODES_H
