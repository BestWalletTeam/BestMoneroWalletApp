...
QPixmap qrCodePlate(const QrCode &qr, const QSize &size, qreal dpr, int radius, int inset) {
    QPixmap plate{size * dpr};
    ...
    const QPixmap code = qr.toPixmap(0);
    if (!code.isNull()) {
        const int span = qMin(size.width(), size.height()) - inset * 2;
        const QRectF dest{(size.width() - span) / 2.0, (size.height() - span) / 2.0,
                          qreal(span), qreal(span)};
        // Nearest-neighbour: smoothing blurs the module edges a scanner needs.
        painter.setRenderHint(QPainter::SmoothPixmapTransform, false);
        painter.drawPixmap(dest, code, QRectF(code.rect()));
    }
    painter.end();
...
QString trimmedDecimal(double amount, int places) {
    QString text = QString::number(amount, 'f', places);
    if (!text.contains('.')) {
        return text;
    }
...
QString dataDirName(const QString &parentDir) {
    static const QString current = QStringLiteral("bestwallet_data");
    static const QString legacy = QStringLiteral("feather_data");

    // Only adopt the old name when a folder under it is actually there.
    if (!QDir(parentDir).exists(current) && QDir(parentDir).exists(legacy)) {
        return legacy;
    }
    return current;
}

QString dataDirPath(const QString &parentDir) {
    return parentDir + "/" + Utils::dataDirName(parentDir);
}

QString portablePath() {
    return Utils::dataDirPath(Utils::applicationPath());
}
...
QString defaultWalletDir() {
    ...
    if (TailsOS::detect()) {
        ...
        QString path = []{
            // An even older layout kept everything in the hidden portable
            // folder named below; prefer it whenever it is still present.
            ...
            if (appImagePath.isEmpty()) {
                ...
                return Utils::dataDirPath(QDir::currentPath()) + "/wallets";
            }
            ...
            return Utils::dataDirPath(absolutePath) + "/wallets";
        }();

        return path;
#else
        if (TailsOS::detectDataPersistence()) {
            return Utils::dataDirPath(QDir::homePath() + "/Persistent") + "/wallets";
        }
#endif
    }
...
QString xdgDesktopEntry(){
    return QString(
            ...
            "Categories=Network;\n"
            "StartupNotify=true\n"
            // Ties the running window to this entry on X11; Wayland uses
            // the desktop file name set in main().
            "StartupWMClass=Best Wallet\n"
...
bool xdgDesktopEntryRegister() {
    ...
    QString iconPathSuffix = "%1/.local/share/icons/bestwallet.png";
    QString iconPath = iconPathSuffix.arg(QDir::homePath());
    if (!fileExists(iconPath)) {
        pixmapWrite(iconPath, appIcon);
    }
    xdgDesktopEntryWrite(QStandardPaths::writableLocation(QStandardPaths::ApplicationsLocation) + "/BestWallet.desktop");
    ...
    if (TailsOS::detect() && TailsOS::detectDotPersistence()) {
        ...
        xdgDesktopEntryWrite(basePath + "/.local/share/applications/BestWallet.desktop");
    }
...
bool isProjectLink(const QString &url) {
    // A browser reads a backslash in the authority as a separator and QUrl does
    // not, so QUrl can see our host where the browser navigates elsewhere.
    if (url.contains(QLatin1Char('\\'))) {
        return false;
    }

    // No guessing at what a malformed URL was meant to be.
    const QUrl parsed{url, QUrl::StrictMode};
    if (!parsed.isValid()) {
        return false;
    }

    // The other way to push the real host out of view.
    if (!parsed.userInfo().isEmpty()) {
        return false;
    }

    // An http:// link could be redirected in transit.
    if (parsed.scheme() != QLatin1String("https")) {
        return false;
    }

    // The parsed host, never the URL text, which can merely contain the domain.
    const QString host = parsed.host().toLower();
    return host == constants::websiteDomain
           || host.endsWith("." + constants::websiteDomain);
}

void externalLinkWarning(QWidget *parent, const QString &url){
    // Our own site isn't a third party, so there is nothing to warn about.
    if (Utils::isProjectLink(url)) {
...
QString getVersion() {
    QString version = QString("%1").arg(BESTWALLET_VERSION);
