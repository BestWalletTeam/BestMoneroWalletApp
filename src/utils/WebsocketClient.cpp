    ...
    , webSocket(new QWebSocket(QString(), QWebSocketProtocol::VersionLatest, this))
{
    ...
    const int urlCount = m_websocketUrls[this->networkType()].length();
    if (urlCount > 0) {
        m_websocketUrlIndex = QRandomGenerator::global()->bounded(urlCount);
...
void WebsocketClient::start() {
    ...
    if (m_url.isEmpty()) {
        // No endpoint configured for the current proxy mode; nothing to do.
        return;
    }
...
void WebsocketClient::nextWebsocketUrl() {
    ...
    const QVector<QUrl> &urls = m_websocketUrls[networkType];
    if (urls.isEmpty()) {
        // No server is configured for this proxy mode. Clear the URL so start()
        // stays a no-op instead of dividing by zero on the rotation below.
        m_url = QUrl();
        return;
    }
    m_websocketUrlIndex = (m_websocketUrlIndex+1)%urls.length();
    m_url = urls[m_websocketUrlIndex];
}
