    ...
    if (m_websocketNodes.count() == 0) {
        ...
        if (nodes_obj.contains(netKey)) {
            ...
            for (auto node: nodes_list) {
                NodeEntry wsNode(node.toString());
                wsNode.custom = false;
                wsNode.online = true;
                m_websocketNodes.append(wsNode);
...
void Nodes::connectToNode(const NodeEntry &node) {
    if (!m_wallet) {
        return;
    }
    ...
    if (useSocks5Proxy(node)) {
        if (conf()->get(Config::proxy).toInt() == Config::Proxy::Tor && (!torManager()->isLocalTor() || torManager()->isAlreadyRunning())) {
            proxyAddress = QString("%1:%2").arg(torManager()->managedTorHost, QString::number(torManager()->managedTorPort));
        } else {
...
void Nodes::autoConnect(bool forceReconnect) {
    ...
    if (wsMode && !m_wsNodesReceived && websocketNodes().count() == 0) {
        ...
        qInfo() << "BestWallet is in websocket connection mode but was not able to receive any nodes (yet).";
        return;
    }
    ...
    if (status == Wallet::ConnectionStatus_Disconnected || forceReconnect) {
        ...
        // try connect
        NodeEntry node = this->pickEligibleNode();
...
NodeEntry Nodes::pickEligibleNode() {
    ...
    for (int index : node_indices) {
        const NodeEntry &node = nodes.at(index);
...
void Nodes::onWSNodesReceived(QList<NodeEntry> &nodes) {
...
void Nodes::setCustomNodes(const QList<NodeEntry> &nodes) {
    m_customNodes.clear();
...
bool Nodes::useSocks5Proxy(const NodeEntry &node) {
    if (node.isLocal()) {
        // When connected to a local node, ignore proxy settings.
        return false;
    }

    if (Utils::isTorsocks()) {
        // When started under torsocks, let torsocks handle traffic.
        return false;
    }
...
void Nodes::resetLocalState() {
    auto resetState = [this](QList<NodeEntry> &model){
        for (auto &node: model) {
...
int Nodes::modeHeight(const QList<NodeEntry> &nodes) {
    QVector<int> heights;
    for (const auto &node: nodes) {
