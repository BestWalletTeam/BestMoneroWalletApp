...
#ifndef BESTWALLET_UTILS_H
#define BESTWALLET_UTILS_H
...
namespace Utils
{
    ...
    // A decimal with trailing zeroes trimmed. Not QString::number's default 'g',
    // which keeps six significant digits and renders small figures in scientific
    // notation -- neither of which is the amount that was asked for. Shared so a
    // figure copied from one screen and pasted into another cannot disagree.
    QString trimmedDecimal(double amount, int places);

    // A QR code centred on a white plate with rounded corners. The code itself is
    // a hard-edged square, so drawn straight onto a rounded label its corners
    // overshoot onto whatever is behind it. `inset` doubles as the code's quiet
    // zone, which is why the code is rendered with no margin of its own.
    ...
    // Data folder used by portable mode and by Tails. An installation that
    // already has one under the pre-rename name keeps using it; see dataDirPath.
    ...
    // Re-resolves stylesheet rules across a widget tree. Qt re-polishes a
    // reparented widget but not its descendants, and uic populates a page before
    // adding it to its container, so anything inside can be polished before its
    // "#Ancestor ..." chain exists. Call once the tree is complete.
