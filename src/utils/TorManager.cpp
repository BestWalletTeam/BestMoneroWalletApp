...
#endif
    if (QString(BESTWALLET_TARGET_TRIPLET) == "arm64-apple-darwin" || QString(BESTWALLET_TARGET_TRIPLET) == "x86_64-apple-darwin") {
        QString appBinaryPath = QCoreApplication::applicationDirPath();
        QDir appBinaryDir(appBinaryPath);
        appBinaryDir.cd("..");
...
void TorManager::init() {
    // Read before shouldStartTorDaemon(), which probes this port.
    managedTorPort = conf()->get(Config::torManagedPort).toString().toUShort();
...
void TorManager::start() {
    ...
    if (Utils::portOpen(managedTorHost, managedTorPort)) {
        this->setErrorMessage(QString("Unable to start Tor on %1:%2. Port already in use.").arg(managedTorHost, QString::number(managedTorPort)));
        return;
    }
    ...
    arguments << "--SocksPort" << QString("%1:%2").arg(managedTorHost, QString::number(managedTorPort));
...
void TorManager::checkConnection() {
    // We might not be able to connect to localhost if torsocks was used to start the wallet
    if (Utils::isTorsocks()) {
        ...
        // Our own daemon: the SOCKS port opens at bootstrap 0%, so the port
        // alone would report a Tor that is nowhere near ready as connected.
        // handleProcessOutput() has the accurate figure.
        this->setConnectionState(m_bootstrapped
                                 && Utils::portOpen(managedTorHost, managedTorPort));
    }

    else {
        // Someone else's Tor: no process output, so the port is all there is.
        this->setConnectionState(Utils::portOpen(managedTorHost, managedTorPort));
    }
}
...
void TorManager::handleProcessOutput() {
    ...
    const QString text = Utils::barrayToString(output);
    this->torLogs.append(text);
    emit logsUpdated();

    // A single read can contain several progress lines, so take the last.
    static const QRegularExpression bootstrapRe(QStringLiteral("Bootstrapped (\\d+)%"));
    auto matches = bootstrapRe.globalMatch(text);
    while (matches.hasNext()) {
        m_bootstrapPercent = matches.next().captured(1).toInt();
    }

    if (m_bootstrapPercent >= 100) {
        qDebug() << "Tor OK";
        m_bootstrapped = true;
        this->setConnectionState(true);
    }
...
bool TorManager::shouldStartTorDaemon() {
    ...
    // Don't start a Tor daemon if BestWallet is run with Torsocks
    if (Utils::isTorsocks()) {
        return false;
    }
    ...
    qWarning() << "BestWallet built without embedded Tor. Assuming --use-local-tor";
    return false;
#endif
    ...
    // Defer to a system Tor when asked to, but only when there is one to defer
    // to; otherwise the wallet pins itself to a dead proxy.
    if (constants::useLocalTorOverride || conf()->get(Config::useLocalTor).toBool()) {
        if (Utils::portOpen(torHost, torPort)) {
            return false;
        }
        qWarning() << QString("Configured to use a local Tor, but nothing is listening on %1. "
                              "Starting the bundled Tor instead.").arg(torHostPort);
    }
