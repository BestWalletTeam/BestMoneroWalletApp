...
void UpdateDialog::noUpdateAvailable() {
    m_waitingTimer.stop();
    this->setStatus("BestWallet is up-to-date.", true);
}
...
void UpdateDialog::updateAvailable() {
    ...
    ui->btn_restart->hide();
    ui->progressBar->hide();
    ui->label_header->setText(QString("New BestWallet version %1 is available").arg(m_updater->version));
...
void UpdateDialog::onInstallUpdate() {
    ...
    if (m_updater->platformTag == "win-installer") {
        this->setStatus("Installer written. Click 'Restart BestWallet' to close BestWallet and start the installer.");
    } else {
        this->setStatus("Installation successful. Do you want to restart BestWallet now?");
    }
    ui->btn_restart->show();
...
void UpdateDialog::installUpdateMac() {
    ...
    QString appName = QString("%1-%2").arg(constants::releaseArtifactName, m_updater->version);
    ...
    m_updatePath = QString("%1/Contents/MacOS/%2").arg(appDir.absolutePath(), constants::macBundleExecutable);
    qDebug() << "Update path: " << m_updatePath;

    file.remove();

    this->setStatus(QString("Installation successful: Do you want to restart BestWallet now?").arg(m_updatePath));
    ui->btn_restart->show();
}
