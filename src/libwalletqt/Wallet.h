...
#ifndef BESTWALLET_WALLET_H
#define BESTWALLET_WALLET_H
...
public:
    ...
    // Attributes this application owns, addressed by bare name. These wrap the
    // raw cacheAttribute calls with the application's key prefix and read
    // through to the pre-rename prefix. See WalletAttributes.h.
    bool setAppAttribute(const QString &name, const QString &val);
    QString getAppAttribute(const QString &name) const;
    bool appAttributeExists(const QString &name);
