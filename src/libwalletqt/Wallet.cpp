        ...
        , m_storeTimer(new QTimer(this))
{
    ...
    m_currentSubaddressAccount = getAppAttribute(WalletAttributes::subaddressAccount).toUInt();
...
void Wallet::switchSubaddressAccount(quint32 accountIndex) {
    if (accountIndex < numSubaddressAccounts())
    {
        ...
        if (!setAppAttribute(WalletAttributes::subaddressAccount, QString::number(m_currentSubaddressAccount)))
        {
            qWarning() << "failed to set" << WalletAttributes::key(WalletAttributes::subaddressAccount) << "cache attribute";
        }
...
qsizetype Wallet::seedLength() const {
    auto seedLength = this->getAppAttribute(WalletAttributes::seed).split(" ", Qt::SkipEmptyParts).length();
...
bool Wallet::setAppAttribute(const QString &name, const QString &val) {
    return this->setCacheAttribute(WalletAttributes::key(name), val);
}

QString Wallet::getAppAttribute(const QString &name) const {
    // Fall back on absence, never on emptiness: an empty string is a real value
    // here, and treating it as "not set" would resurrect the pre-rename list.
    std::string value;
    if (m_wallet2->get_attribute(WalletAttributes::key(name).toStdString(), value)) {
        return QString::fromStdString(value);
    }
    if (m_wallet2->get_attribute(WalletAttributes::legacyKey(name).toStdString(), value)) {
        return QString::fromStdString(value);
    }
    return {};
}

bool Wallet::appAttributeExists(const QString &name) {
    return this->cacheAttributeExists(WalletAttributes::key(name))
           || this->cacheAttributeExists(WalletAttributes::legacyKey(name));
}
