...
void signal_handler(int signum) {
    ...
    if (conf()->get(Config::writeStackTraceToDisk).toBool()) {
        ...
        out << QString("Version: %1\n").arg(BESTWALLET_VERSION).toStdString();
...
if (AttachConsole(ATTACH_PARENT_PROCESS)) {
    ...
    QApplication::setApplicationName("Best Wallet");
    // Matches the entry written by Utils::xdgDesktopEntryRegister(); without it
    // docks and app switchers show a generic icon.
    QGuiApplication::setDesktopFileName("BestWallet");
    QApplication::setApplicationVersion(BESTWALLET_VERSION);

    QCommandLineParser parser;
    parser.setApplicationDescription("Best Wallet - a free Monero desktop wallet");
    ...
    if (app.isAlreadyRunning()) {
        qWarning() << "Another instance of Best Wallet is already running";
        return EXIT_SUCCESS;
    }
    ...
    Monero::Wallet::init("", "bestwallet", logPath.toStdString(), true);
    ...
    // Applies to this run only -- see constants::useLocalTorOverride.
    if (parser.isSet("use-local-tor"))
        constants::useLocalTorOverride = true;

    conf()->set(Config::restartRequired, false);

    if (!quiet) {
        QList<QPair<QString, QString>> info;
        info.emplace_back("BestWallet", BESTWALLET_VERSION);
