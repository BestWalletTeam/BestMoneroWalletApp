...
void NodeModel::updateNodes(const QList<NodeEntry> nodes) {
    beginResetModel();
    m_nodes.clear();
    m_nodes = nodes;
    endResetModel();
...
QVariant NodeModel::data(const QModelIndex &index, int role) const {
    ...
    NodeEntry node = m_nodes.at(index.row());
        ...
        switch (index.column()) {
            case NodeModel::URL: {
                if (node.isConnecting)
                    return QString("Best Wallet is connecting to this node.");
                if (node.isActive)
                    return QString("Best Wallet is connected to this node.");
            }
        }
    }
...
NodeEntry NodeModel::node(int row) {
