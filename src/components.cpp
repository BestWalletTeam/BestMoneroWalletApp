...
StatusBarButton::StatusBarButton(const QIcon &icon, const QString &tooltip, QWidget *parent) : QPushButton(parent) {
    ...
    setToolTip(tooltip);
    setFlat(true);
    // A square hit target a little larger than the glyph. What a glyph does
    // inside the box is normalised by Icons::fitted.
    setObjectName("statusBarButton");
    setFixedSize(kStatusBarButtonSize, kStatusBarButtonSize);
    setIconSize(QSize(kStatusBarIconSize, kStatusBarIconSize));
