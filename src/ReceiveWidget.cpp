        ...
        , m_wallet(wallet)
{
    ...
    connect(ui->btn_copyAddress, &QToolButton::clicked, this, &ReceiveWidget::copyAddress);
...
void ReceiveWidget::generateSubaddress() {
    ...
    // The list may be long enough to scroll the new address out of view.
    const int lastRow = m_proxyModel->rowCount() - 1;
    if (lastRow < 0) {
        return;
    }

    const QModelIndex index = m_proxyModel->index(lastRow, 0);
    ui->addresses->setCurrentIndex(index);
    ui->addresses->scrollTo(index);
}
...
void ReceiveWidget::updateQrCode(){
    ...
    if (!index.isValid()) {
        ...
        ui->label_currentAddress->clear();
        ui->btn_copyAddress->setEnabled(false);
        return;
    }
    ...
    if (qrc.isValid()) {
        ...
        ui->qrCode->setPixmap(Utils::qrCodePlate(qrc, ui->qrCode->size(),
                                                 this->devicePixelRatioF(), 16, 14));
        ui->btn_createPaymentRequest->show();
    }

    ui->label_currentAddress->setText(address);
    ui->btn_copyAddress->setEnabled(true);
}
