...
{
    ui->setupUi(this);
    this->setTitle("Welcome to Best Wallet");
    ...
    connect(m_portOpenWatcher, &QFutureWatcher<QPair<bool, QString>>::finished, [this](){
        ...
        if (nodeFound) {
            ...
            ui->label_nodeDetected->setText(QString("Best Wallet detected a local node on %1").arg(res.second));
...
bool PageNetwork::isComplete() const {
    ...
    NodeEntry node{ui->line_customNode->text()};
    return node.isValid();
}
