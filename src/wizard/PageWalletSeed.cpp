    ...
    , m_fields(fields)
{
    ...
    ui->frame_notice->setInfo(icons()->icon("seed"), "The following **16** words can be used to recover access to your wallet.\n\n"
                                                   ...
                                                   "Best Wallet uses **Polyseed**. For more information click **Help**.");

    ui->frame_invalidSeed->setInfo(icons()->icon("warning"), "Best Wallet was unable to generate a valid seed.\n"
...
bool PageWalletSeed::validatePage() {
    ...
    auto *btn_understand = seedWarning.addButton("I understand", QMessageBox::AcceptRole);

    // Neither choice should be a small target on a point-of-no-return dialog.
    btn_understand->setMinimumWidth(150);
