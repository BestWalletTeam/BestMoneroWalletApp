...
APP_ID="org.bestwallet.BestWallet"
...
mkdir export
cp -r /bestwallet/contrib/flatpak/share export
rm -rf export/share/app-info

# Copy the metadata file
cp /bestwallet/contrib/flatpak/metadata .

mkdir files
cd files

# Copy flatstart binary
cp /bestwallet/contrib/depends/x86_64-linux-gnu/bin/startup .

# Copy BestWallet binary
cp /outdir/BestWallet BestWallet

# Copy metadata
cp -r /bestwallet/contrib/flatpak/share .
...
# Copy icons
mkdir -p share/icons
cp -r /bestwallet/contrib/flatpak/icons/gnome share/icons
...
# Patch the wallet binary
patchelf --set-interpreter "/${GUIX_GLIBC}/lib/ld-linux-x86-64.so.2" BestWallet
patchelf --set-rpath "/${GUIX_GLIBC}/lib:/${GUIX_FONTCONFIG}/lib:\$ORIGIN/lib" BestWallet
