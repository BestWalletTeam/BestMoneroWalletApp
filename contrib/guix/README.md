# Bootstrappable BestWallet builds

This directory holds everything needed to produce [bootstrappable](https://bootstrappable.org) BestWallet release builds.
...
Packages Guix does not carry can be defined in the [manifest](manifest.scm) or upstreamed.

Releases are independently rebuilt and cryptographically attested to by more than one contributor, so that no single builder has to be trusted.
Attestations are submitted to the signatures repository referenced by the `external/bestwallet-sigs` submodule.
...
```bash
$ git clone https://github.com/BestWalletTeam/BestWalletApp
$ cd BestWallet
```
...
To reproducibly build BestWallet with the default options, run this from the top of a
clean repository:
...
With the signatures repository cloned, attest to the commit or tag currently
checked out:
...
Once at least one other signer has pushed their signatures to the signatures
repository:
