...
(
    ...
    CMAKEVARS="-DWITH_SCANNER=On -DCHECK_UPDATES=On -DSELF_CONTAINED=On -DBESTWALLET_TARGET_TRIPLET=${HOST} -DWITH_PLUGIN_REDDIT=Off"

    # CHANGENOW_API_KEY stays in the environment so a traced build never prints it.
    if [[ -n "${TAG}" ]]; then
        CMAKEVARS+=" -DOFFICIAL_BUILD=On"
    else
        unset CHANGENOW_API_KEY
    fi
    ...
    case "$HOST" in
        *linux*)
            if [ "$OPTIONS" != "pack" ]; then
                ...
                mv BestWallet.AppImage "${APPIMAGENAME}"
        ...
        *mingw*)
            case "$OPTIONS" in
                installer)
                    ...
                    cp contrib/installers/windows/BestWalletSetup-*.exe "${INSTALLPATH}/"
                    mv contrib/installers/windows/BestWalletSetup-*.exe "${OUTDIR}/"
                    ;;
            esac
            ;;
    esac

    # Install built BestWallet to $INSTALLPATH
    case "$HOST" in
        *darwin*)
        ...
        *)
            case "$OPTIONS" in
                installer)
                    # do nothing, we don't want the .exe in the final .zip
                    ;;
                *)
    ...
    (
        ...
        case "$HOST" in
            *darwin*)
                mv "BestWallet.app" "BestWallet.app"
                mkdir -p BestWallet.app/Contents/bin
                cp -a /bestwallet/contrib/depends/${HOST}/Tor/libevent-2.1.7.dylib BestWallet.app/Contents/bin
                cp -a /bestwallet/contrib/depends/${HOST}/Tor/tor BestWallet.app/Contents/bin
                chmod +x BestWallet.app/Contents/bin/tor
                ;;
        esac
            ...
            *mingw*)
                case "$OPTIONS" in
                    installer)
                        ...
                        ;;
                    "")
                        mv BestWallet.exe ${DISTNAME}.exe && \
                        find . -print0 \
                            | xargs -0r touch --no-dereference --date="@${SOURCE_DATE_EPOCH}"
                        find . \
                            | sort \
